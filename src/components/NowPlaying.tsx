...
const NowPlaying: React.FC = () => {
    ...
    useEffect(() => {
        const fetchNowPlaying = async () => {
            try {
                const response = await fetch(`https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=zx111&api_key=${secrets.REACT_APP_LASTFM_API_KEY}&format=json&limit=1&nowplaying=true`);
